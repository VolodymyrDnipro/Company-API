...
async def get_user_rating_by_all_quizzes_by_date(
        start_date: date = Query(..., description="2010-02-02"),
        end_date: date = Query(..., description="2020-02-02"),
        user_email: str = Depends(get_user_data),
        analytics_service: AnalyticsService = Depends(get_analytics_service),
):
    try:
...
async def company_get_average_scores_by_user_id(
        company_id: int = Path(),
        user_id: int = Path(),
        start_date: date = Query(..., description="2010-02-02"),
        end_date: date = Query(...,  description="2020-02-02"),
    ...
    try:
        ...
        user_rating = await analytics_service.company_get_average_scores_by_user_id(user_id, company_id, start_date, end_date)
    except HTTPException as exc:
        raise exc
    return user_rating
